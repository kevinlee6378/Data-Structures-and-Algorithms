...
public class ShortestPaths {
	...
	public void run() {
		...
		while (pq.isEmpty() != true) {
			...
			for (Edge e : min.getVertex().edgesFrom()) {
				...
				if ((map.get(e.from).getValue().getDistance() + weights.get(e)) < d.getValue().getDistance()) {
					d.decrease(d.getValue().sameVertexNewDistance((map.get(e.from).getValue().getDistance() + weights.get(e))));
					toEdge.put(e.to, e);					
				}
	...
	public LinkedList<Edge> returnPath(Vertex endVertex) {
		...
		Vertex v = endVertex;
		while (map.get(v).getValue().getDistance() != 0) {
